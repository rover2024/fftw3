/****************************************************************************
** CFI wrapped code from reading C file 'vrank-geq1-rdft2__cfic_tmp_new__.c'
**
...
static void apply(const plan *ego_, R *r0, R *r1, R *cr, R *ci)
{
     ...
     for (i = 0; i < vl; ++i) {
          LORELIB_CFI_7(cldapply)(ego->cld, r0 + i * rvs, r1 + i * rvs,
		   cr + i * cvs, ci + i * cvs);
     }
}
...
static void print(const plan *ego_, printer *p)
{
     ...
     LORELIB_CFI_27(p->print)(p, "(rdft2-vrank>=1-x%D/%d%(%p%))",
