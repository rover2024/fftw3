/****************************************************************************
** CFI wrapped code from reading C file 'hc2hc-generic__cfic_tmp_new__.c'
**
...
static void apply_dit(const plan *ego_, R *IO)
{
     ...
     LORELIB_CFI_6(cld0->apply)(ego->cld0, IO, IO);

     start = ego->mstart1 * ego->s;
     cld = (plan_rdft *) ego->cld;
     LORELIB_CFI_6(cld->apply)(ego->cld, IO + start, IO + start);
...
{
     const P *ego = (const P *) ego_;
     LORELIB_CFI_27(p->print)(p, "(hc2hc-generic-%s-%D-%D%v%(%p%)%(%p%))", 
...
static void regsolver(planner *plnr, INT r)
{
     ...
     if (X(mksolver_hc2hc_hook)) {
	  slv = (S *)fftw_mksolver_hc2hc_hook(sizeof(S), r, mkcldw);
	  REGISTER_SOLVER(plnr, &(slv->super));
     }
}
