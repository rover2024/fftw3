/****************************************************************************
** CFI wrapped code from reading C file 'indirect__cfic_tmp_new__.c'
**
...
static void apply_before(const plan *ego_, R *ri, R *ii, R *ro, R *io)
{
...

     {
          ...
          LORELIB_CFI_7(cldcpy->apply)(ego->cldcpy, ri, ii, ro, io);
     }
     {
...
static void apply_after(const plan *ego_, R *ri, R *ii, R *ro, R *io)
{
     ...
     {
          plan_dft *cld = (plan_dft *) ego->cld;
          LORELIB_CFI_7(cld->apply)(ego->cld, ri, ii, ri, ii);
     }
     {
          plan_dft *cldcpy = (plan_dft *) ego->cldcpy;
          LORELIB_CFI_7(cldcpy->apply)(ego->cldcpy, ri, ii, ro, io);
...
static void print(const plan *ego_, printer *p)
{
     const P *ego = (const P *) ego_;
     const S *s = ego->slv;
     LORELIB_CFI_27(p->print)(p, "(%s%(%p%)%(%p%))", s->adt->nam, ego->cld, ego->cldcpy);
}
