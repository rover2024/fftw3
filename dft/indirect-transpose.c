/****************************************************************************
** CFI wrapped code from reading C file 'indirect-transpose__cfic_tmp_new__.c'
**
...
static void apply_op(const plan *ego_, R *ri, R *ii, R *ro, R *io)
{
     ...
     for (i = 0; i < vl; ++i) {
	  {
	       ...
	       LORELIB_CFI_7(cldtrans->apply)(ego->cldtrans, ri, ii, ro, io);
	  }
	  {
	       plan_dft *cld = (plan_dft *) ego->cld;
	       LORELIB_CFI_7(cld->apply)(ego->cld, ro, io, ro, io);
	  }
	  ri += ivs; ii += ivs;
	  ro += ovs; io += ovs;
     }
     {
	  ...
	  LORELIB_CFI_7(cldrest->apply)(ego->cldrest, ri, ii, ro, io);
     }
}
...
{
     const P *ego = (const P *) ego_;
     LORELIB_CFI_27(p->print)(p, "(indirect-transpose%v%(%p%)%(%p%)%(%p%))", 
